...
class RequestResponseActivity : AppCompatActivity() {
                ...
                call.enqueue(object : Callback<String> {
                    override fun onResponse(call: Call<String>, response: Response<String>) {
                        if (response.isSuccessful) {
                            ...
                            startActivity(intent)
                            Toast.makeText(
                                applicationContext, "Request accepted", Toast.LENGTH_SHORT
                            ).show()
                        } else {
                            Log.d(
                                "ERROR", "Request respond failed. Response code: ${response.code()}"
                            )
                            Toast.makeText(
                                applicationContext,
                    ...
                    override fun onFailure(call: Call<String>, t: Throwable) {
                        Log.d("ERROR", "Request respond failed serverside. Error: ${t.message}")
                        Toast.makeText(
            ...
            call.enqueue(object : Callback<String> {
                override fun onResponse(call: Call<String>, response: Response<String>) {
                    if (response.isSuccessful) {
                        val intent = Intent(applicationContext, DoctorDashboardActivity::class.java)
                        startActivity(intent)
                        Toast.makeText(applicationContext, "Request declined", Toast.LENGTH_SHORT)
                            .show()
                    } else {
                        Log.d("ERROR", "Request decline failed. Response code: ${response.code()}")
                        Toast.makeText(
                            applicationContext,
                            "Request decline failed. Check the fields!",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
                ...
                override fun onFailure(call: Call<String>, t: Throwable) {
                    Log.d("ERROR", "Request decline failed serverside. Error: ${t.message}")
                    Toast.makeText(
                        applicationContext, "Server error. Try again!", Toast.LENGTH_SHORT
                    ).show()
                }
